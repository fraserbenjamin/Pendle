    ...
    if(state.currentQuestion >= 0 && questions?.list?.length > 0 && teams?.list && teamId) {
        ...
        return (
            <Container>
                ...
                <Options>
                    <Option selected={teams.list[teamId][state.currentQuestion] === 1} onClick={() => setGuess(teamId, state.currentQuestion, 1)}>{question.option1}</Option>
                    <Option selected={teams.list[teamId][state.currentQuestion] === 2} onClick={() => setGuess(teamId, state.currentQuestion, 2)}>{question.option2}</Option>
                    <Option selected={teams.list[teamId][state.currentQuestion] === 3} onClick={() => setGuess(teamId, state.currentQuestion, 3)}>{question.option3}</Option>
                    <Option selected={teams.list[teamId][state.currentQuestion] === 4} onClick={() => setGuess(teamId, state.currentQuestion, 4)}>{question.option4}</Option>
                </Options>
            </Container>
        );
    }
